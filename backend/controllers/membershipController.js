...
const submitMembership = async (req, res) => {
  try {
    ...
    // Send acknowledgment email to applicant if transporter available
    if (transporter) {
      const mailOptions = {
      ...
      try {
        ...
        console.log(`Membership acknowledgment email sent to ${email}`);
      } catch (emailErr) {
        console.error("Failed to send membership acknowledgment email:", emailErr);
        // Log but don't fail the response
      }
    } else {
      console.warn("Email transporter not available - membership email skipped");
    }
...
const approveMembership = async (req, res) => {
  try {
    ...
    // Send approval email if transporter available
    if (transporter) {
      const mailOptions = {
        from: `"Shiva Temple" <${EMAIL_USER}>`,
        to: membership.email,
      ...
      try {
        ...
        console.error("Failed to send membership approval email:", emailErr);
        // Log but don't fail the response
      }
    } else {
      console.warn("Email transporter not available - membership approval email skipped");
    }
