...
// Book darshan - direct booking with auto-confirmation and email (no admin approval)
const bookDarshan = async (req, res) => {
  try {
    ...
    // Insert darshan booking with direct confirmed status (no pending)
    const darshanCollection = getDarshanCollection();
    const result = await darshanCollection.insertOne({
      ...
      totalCost,
      status: "confirmed", // Direct confirmation as per requirements
      bookedAt: new Date(),
      confirmedAt: new Date(),
    });

    // Send confirmation email immediately if transporter available
    if (transporter) {
      const mailOptions = {
      ...
      try {
        ...
        console.log(`Darshan confirmation email sent to ${email}`);
      } catch (emailErr) {
        console.error("Failed to send darshan confirmation email:", emailErr);
        // Log but don't fail the response
      }
    } else {
      console.warn("Email transporter not available - darshan email skipped");
    }

    res.status(201).json({ 
      success: true, 
      message: "Darshan booked and confirmed successfully", 
...
// Deprecated - no longer needed as bookings are direct
const confirmDarshan = async (req, res) => {
  try {
    res.status(400).json({ 
      message: "Darshan bookings are automatically confirmed upon submission" 
    });
  } catch (err) {
