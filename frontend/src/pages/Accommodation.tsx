...
// // Available room types with pricing and capacity details
...
//   // Calculate total booking price including extra guest charges
...
//   // Fetch date availability from API for selected room type
//   const fetchDateAvailability = async () => {
//     try {
//       const selectedRoomData = roomTypes.find((room) => room.id === selectedRoom);
//       const roomTypeName = selectedRoomData ? selectedRoomData.name : '';
//       const url = roomTypeName 
//         ? `${API_URL}/api/accommodations/availability?roomType=${encodeURIComponent(roomTypeName)}`
//         : `${API_URL}/api/accommodations/availability`;
...
//   // Validate selected dates and check availability with API
...
//     try {
//       const response = await fetch(`${API_URL}/api/accommodations/check`, {
//         method: 'POST',
//         headers: {
...
//       if (!isAvailable) {
//         setError(`Selected dates are not available for ${roomTypeName}. Please choose different dates.`);
//       }
//       return isAvailable;
//     } catch (err) {
...
//   // Update availability when room selection changes
//   useEffect(() => {
//     if (selectedRoom) {
...
//   // Validate dates when room is selected and dates are present
//   useEffect(() => {
//     if (step === 1 && selectedRoom && checkIn && checkOut) {
//       validateDates();
//     }
//   }, [checkIn, checkOut, step, selectedRoom]);
...
//   // Apply CSS classes to calendar dates based on availability
...
//         <div className="text-center">
//           <h1 className="text-4xl font-bold tracking-tight text-gray-900 sm:text-5xl bg-clip-text text-transparent bg-gradient-to-r from-orange-600 to-yellow-500">
...
//         {selectedRoom && checkIn && checkOut && isDateAvailable !== null && (
...
//                                 className="mt-2 block w-full h-12 px-4 py-3 rounded-xl border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 text-base font-medium transition-all duration-300"
//                                 required
//                               />
//                             </div>
//                           ))}
...
//                                 className="block w-full h-12 pl-4 pr-12 rounded-xl border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 text-base transition-all duration-300"
//                                 required
...
//                               className="block w-full h-12 pl-4 pr-12 rounded-xl border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 text-base appearance-none transition-all duration-300"
//                               required
//                             >
//                               {[1, 2, 3, 4].map((num) => (
...
//                               ))}
//                             </select>
//                             <Users className="absolute right-4 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-400 pointer-events-none" />
//                           </div>
//                         </div>
//                       </div>
//                     </>
...
//                           className="mt-2 block w-full h-12 px-4 py-3 rounded-xl border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 text-base transition-all duration-300"
...
export function Accommodation() {
  ...
  const validateDates = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/accommodations/check-range`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.error('Error checking availability:', err);
      setError(err.message || 'Failed to check date availability. Please try again.');
      return false;
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (step === 0 && agreed) {
      ...
      if (paymentMethod === 'upi' && !cardNumber) { // Using cardNumber field for UPI ID
      ...
      try {
        const response = await fetch(`${API_URL}/api/accommodations/book`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        setBookingId(result.id || result.bookingId || 'BOOK123');
        setSuccessMessage('Booking request submitted successfully! Your booking is pending admin confirmation.');
        setBookingComplete(true);
        setError('');
      } catch (err: any) {
        console.error('Booking error:', err);
      ...
      <div className="min-h-screen bg-gradient-to-b from-orange-50 to-white py-12">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            {successMessage && (
              <div className="mt-4 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded animate-fade-in">
                {successMessage}
              </div>
            )}
          </div>
                          ...
                          <div className="relative">
                            ...
                            <div className="relative">
                              <DatePicker
                                ...
                                minDate={today}
                                dateFormat="yyyy-MM-dd"
                                className="block w-full h-12 pl-4 pr-12 rounded-xl border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 text-base transition-all duration-300 disabled:opacity-50"
                                required
                                dayClassName={getDayClassName}
                                placeholderText="Select check-in date"
                                disabled={isSubmitting}
                        ...
                        <div className="relative max-w-xs">
                          ...
                          <div className="relative">
                            <select
                              ...
                              value={guests}
                              onChange={(e) => setGuests(e.target.value)}
                              className="block w-full h-12 pl-4 pr-12 rounded-xl border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 text-base appearance-none transition-all duration-300 disabled:opacity-50"
                              required
                              disabled={isSubmitting}
                            >
                              {[1, 2, 3, 4].map((num) => (
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={cardNumber}
                          onChange={handleCardNumberChange}
                          className="mt-2 block w-full h-12 px-4 py-3 rounded-xl border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 text-base transition-all duration-300 disabled:opacity-50"
                          placeholder="1234 5678 9012 3456"
                          maxLength={19}
                          required
                          disabled={isSubmitting}
                  ...
                  {paymentMethod === 'upi' && (
                    <div>
                      ...
                      <input
                        type="text"
                        id="upi-id"
                        value={cardNumber} // Reuse cardNumber field for UPI
                        onChange={handleCardNumberChange}
                        className="mt-2 block w-full h-12 px-4 py-3 rounded-xl border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 text-base transition-all duration-300 disabled:opacity-50"
                        placeholder="username@upi"
                        required
              ...
              {step !== 0 && (
                <div className="flex space-x-4 pt-4" >
                  <button
                    type="submit"
                    disabled={isSubmitting}
                    className={`flex-1 rounded-xl px-6 py-4 text-base font-semibold text-white shadow-md transition-all duration-300 ${
                      isSubmitting 
                        ? 'bg-gray-400 cursor-not-allowed' 
                        : 'bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600'
                    }`}
                  >
                    {isSubmitting ? 'Submitting...' : (step === 1 ? 'Continue to Payment' : 'Submit Booking Request')}
                  </button>
                  {step === 2 && (
                    <button
                      type="button"
                      onClick={() => setStep(1)}
                      disabled={isSubmitting}
                      className="flex-1 rounded-xl border-2 border-gray-300 px-6 py-4 text-base font-semibold text-gray-700 shadow-sm hover:bg-gray-50 transition-all duration-300 disabled:opacity-50"
                    >
                      Back to Room Selection
                    </button>
                  )}
