          ...
          <div className="p-8 animate-fade-in">
            ...
            <div className="bg-white p-8 rounded-2xl shadow-xl">
              ...
              {!loading && poojas.length === 0 && (
                <p className="text-gray-500">No poojas_available</p>
              )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {getBookedDates().map((range, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`p-4 border rounded-xl hover:shadow-lg transition-all duration-300 ${
                    isBooked ? "border-red-200 bg-red-50" : "border-green-200 bg-green-50"
                  }`}
                >
                  <p className="font-semibold text-orange-700">
                    From: {range.start}
                  ...
                  onClick={async () => {
                    try {
                      const response = await fetch(
                        `http://localhost:3000/api/accommodations/confirm/${booking._id}`,
                        {
                          method: "POST",
                          headers: {
                            "Content-Type": "application/json",
              ...
              <div className="bg-white p-8 rounded-2xl shadow-xl mb-8">
                ...
                {!loading && darshanBookings.length === 0 && (
                  <p className="text-gray-500">
                    No bookings yet to show availability.
                  </p>
                )}
                {!loading && darshanBookings.length > 0 && (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
